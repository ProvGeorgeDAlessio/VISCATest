...

{
	...
	VISCA_Cam * Cam1 = new VISCA_Cam("COM1",1,1,result);
	VISCA_Cam * Cam2 = new VISCA_Cam("COM2",1,2,result);
	...
	if ((Cam1!= 0) && (Cam2!= 0))
	{
		...
		std::cout << "Cam2: Camera Type: " << cammod << " (Name: " << Cam2->cam_model_name << ") Found \n";
		...
		while (zoomlevel >= 0)
		{
			...
			if (zoomlevel >=0)
			{
				...
				xx=Cam2->set_zoom_direct(zoomlevel,1);
			}
			else if (zoomlevel == -2)
			{
				xx=  Cam1->set_wb_onepush(1);
				xx = Cam2->set_wb_onepush(1);
				zoomlevel = 0;
			}
			...
			if (zoomlevel == -4)
			{
				...
				if (irmode == 0)
				{
					...
					xx = Cam2->enable_ir_mode(1,true);
					irmode = 1;
				}
				else
				{
					xx = Cam1->enable_ir_mode(1,false);
					xx = Cam2->enable_ir_mode(1,false);
					irmode = 0;
			   }
			}
		}
